...
### Added
- **"When blocked: block screen, or just leave" — a new choice on every in-app feature rule.** Blocking Reels, Shorts, Explore or the TikTok feed has always meant the full block screen, whose only way out is your home screen — so a rule about *one part* of an app threw you out of the whole app. Pick "Just leave" and the block simply backs you out of that feature and leaves you where you were: swipe to the next reel and you're back in your DMs, still in Instagram, no block screen and no trip to the launcher. The block screen is still the default and still there if you want the harder stop, and it takes over automatically if leaving the feature doesn't work. Daily time limits always use the block screen: running out of time for the day is about the whole app, not one corner of it. Under Strict Mode, switching a rule to "Just leave" asks for the unlock challenge, since it is the softer stop.
...
### Fixed
- **Nudge now tells you when it has stopped blocking.** Android can stop the accessibility service on its own — when it needs memory, after a backup, or at a phone maker's discretion — and it does not always start it again. Until now Nudge had no idea: it kept reporting "Nudge is active" while enforcing nothing, and there was no way for you to find out except by noticing that an app you had blocked opened normally. Nudge now checks, in the background, whether it is genuinely still running rather than merely still switched on — the two are not the same, and Android's own Settings screen will keep showing Nudge as enabled long after it has stopped. If blocking is down, you get a notification saying so, with the steps to start it again.
- **Nudge's Settings screen no longer shows a green tick over a service that has stopped** — and, worse, no longer tells you to switch off a service that is working. It read the permission once when the screen opened and never looked again, so it could sit on a stale answer indefinitely. Turning the service back on now clears the warning within a couple of seconds, without leaving the screen.
- **Nudge is no longer backed up to Google Drive.** Nudge has never asked for internet access and its data has always been meant to stay on your phone, but Android's automatic backup was uploading the database anyway, on a schedule nobody chose. That is now switched off. Running the backup also killed the app for a few seconds each time, which is long enough for the "Nudge is active" notification to be telling you something untrue; that window is gone with it. To move your rules to a new phone, use Settings, Export rules, and import the file on the other side.
...
- **The background service now follows the on/off switch, and starts when it should.** It only ever started after a reboot — so a fresh install had nothing running until you next restarted your phone, and every app update left it stopped until you did. It now starts when you open Nudge, when you switch blocking on, and after Nudge updates itself. Once running it also never stopped, so its "Nudge is active" notification stayed up after you switched Nudge off; it now goes away with the switch.
